...
export async function handleWebhookEvent(req, res) {
  try {
    ...
    // Return error details in response for debugging (remove or sanitize in production)
    res.status(500).json({ error: String(err), stack: err && err.stack ? err.stack.split('\n').slice(0,10).join('\n') : undefined });
  }
}
